...
newtype TypeCheckM a = TypeCheckM {runTypeCheckM :: ExceptT Error (RWS Env () Tag) a}
  deriving
    ( Functor,
      Applicative,
      Monad,
...
instance MonadSymTable TypeCheckM EnvEntry where
  askSymTable = asks envVal
  withSymTable f = local $ \env -> env {envVal = f $ envVal env}

instance MonadSymTable TypeCheckM Ty where
...
lookupSym :: String -> TypeCheckM (Maybe Symbol)
lookupSym s = (M.!? s) <$> asks envSym

lookupSym' :: String -> Maybe SourcePos -> TypeCheckM Symbol
lookupSym' s pos = do
  msym <- lookupSym s
  case msym of
...
lookupVar :: Symbol -> Maybe SourcePos -> TypeCheckM Ty
...
lookupFun :: Symbol -> Maybe SourcePos -> TypeCheckM ([Ty], Ty)
...
lookupTy :: Symbol -> Maybe SourcePos -> TypeCheckM Ty
lookupTy sym pos = do
  mty <-
    runMaybeT $
...
lookupTyNoUnpack :: Symbol -> Maybe SourcePos -> TypeCheckM Ty
...
withSym :: String -> (Symbol -> TypeCheckM a) -> TypeCheckM a
withSym s m = do
...
checkVar :: UntypedVar -> TypeCheckM (Var ::: Ty)
...
checkExp (ForExp i from to b pos) = do
  ...
  withSym i $ \i_sym -> do
    b' ::: b_t <- insertSym i_sym (VarEntry Int) $ checkExp b
...
checkDecs :: [UntypedDec] -> ([Dec] -> TypeCheckM a) -> TypeCheckM a
...
checkDec :: UntypedDec -> (Dec -> TypeCheckM a) -> TypeCheckM a
checkDec (FunctionDec decs) m =
  ...
  where
    ...
    withParams :: [UntypedField] -> ([Field ::: Ty] -> TypeCheckM a) -> TypeCheckM a
...
checkDec (TypeDec decs) m =
  ...
  where
    ...
    checkTy :: UntypedTy -> (AST.Ty Symbol ::: Ty -> TypeCheckM a) -> TypeCheckM a
    ...
    checkTy (RecordTy fields) m = do
      ...
      where
        ...
        checkField :: UntypedField -> (Field ::: Ty -> TypeCheckM a) -> TypeCheckM a
...
compatTypes :: SourcePos -> Ty -> Ty -> TypeCheckM ()
